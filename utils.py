...
def plot_race_summary(year, grand_prix, session, driver_1, driver_2):
    ...
    fig = fig.add_trace(go.Scatter(x=laps_driver_1['LapNumber'], 
                                    ...
                                    mode='lines', 
                                    name=driver_1,
                                    hovertemplate = 'Lap: %{x},<br>LapTime: %{y}sec'
                                    )
                        )
    ...
    # Edit the layout
    fig.update_layout(title=f'{session_data.event.EventName} {session_data.event.year} {session} Summary for {driver_1} and {driver_2}',
...
def plot_driver_comparison_for_lap_number(year, grand_prix, session, driver_1, driver_2, lap_number):
    ...
    # Edit the layout
    fig.update_layout(title=f'Comparing Speed of {driver_1} and {driver_2} through Lap {lap_number}',
...
@st.cache(show_spinner=False)
def plot_fastest_session_lap_comparison(year, grand_prix, session, driver_1, driver_2):
    '''
    INPUT:
    year: year of the season; data type = int
    grand_prix: name of the grand_prix; data type = str
    ...
    # Edit the layout
    fig.update_layout(title=f'Comparing Speed of {driver_1} and {driver_2} through the Fastest {session} Lap',
                    xaxis_title='Distance in m',
                    yaxis_title='Speed in km/h')
